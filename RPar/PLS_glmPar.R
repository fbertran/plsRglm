    ...
    for (kk in 1:nt)
    {
        ...
        if (modele %in% c("pls-glm-family", "pls-glm-Gamma", "pls-glm-gaussian",
            ...
            if (!pvals.expli)
            {
              ...
              tempww =  foreach(jj=1:(res$nc), .combine=c) %dopar% {
                  coef(glm(YwotNA ~ cbind(res$tt, XXwotNA[,jj]), family = family))[kk + 1]
                }
                XXwotNA[!XXNA] <- 0
            ...
            } else
            {
                ...
                tmpList <- foreach(jj=1:(res$nc), .combine='cbind',.multicombine=TRUE) %dopar% {
                  tmww <- summary(glm(YwotNA ~ cbind(res$tt, XXwotNA[,
                    jj]), family = family))$coefficients[kk + 1, ]
        ...
        if (modele %in% c("pls-glm-polr"))
        {
            ...
            if (!pvals.expli)
            {
                ...
                for (jj in 1:(res$nc))
                {
                  tempww[jj] <- -1 * MASS::polr(YwotNA ~ cbind(tts, XXwotNA[,
