PLS_glm <- function(dataY, dataX, nt = 2, limQ2set = 0.0975, dataPredictY = dataX,
    ...
    if (any(apply(is.na(dataX), MARGIN = 2, "all")))
    {
        return(vector("list", 0))
        cat("One of the columns of dataX is completely filled with missing data\n")
        stop()
    }
    if (any(apply(is.na(dataX), MARGIN = 1, "all")))
    {
        return(vector("list", 0))
        cat("One of the rows of dataX is completely filled with missing data\n")
        stop()
    }
    if (identical(dataPredictY, dataX))
    {
        PredYisdataX <- TRUE
    } else
    {
    ...
    if (!PredYisdataX)
    {
        # if(any(apply(is.na(dataPredictY),MARGIN=2,'all'))){return(vector('list',0));
        # cat('One of the columns of dataPredictY is completely filled with
        # missing data\n'); stop()}
        if (any(apply(is.na(dataPredictY), MARGIN = 1, "all")))
        {
            return(vector("list", 0))
            cat("One of the rows of dataPredictY is completely filled with missing data\n")
            stop()
        }
    }
    if (missing(weights))
    ...
    {
        if (all(weights == rep(1, length(dataY))))
        {
            NoWeights <- TRUE
        } else
        {
    ...
    {
        na.miss.X <- TRUE
    } else na.miss.X <- FALSE
    if (any(is.na(dataY)))
    {
        na.miss.Y <- TRUE
    } else na.miss.Y <- FALSE
    if (any(is.na(dataPredictY)))
    {
        na.miss.PredictY <- TRUE
    } else
    {
    ...
    } else
    {
        ...
        } else
        {
            if (!missing(naive))
            {
                if (verbose)
                {
                  cat(paste("Only naive DoF can be used with PLS GLM\n",
                    sep = ""))
                }
            }
            naive <- TRUE
    ...
    if (na.miss.X | na.miss.Y)
    {
        ...
        if (verbose)
        {
            cat(paste("Only naive DoF can be used with missing data\n",
                sep = ""))
        }
        if (!NoWeights)
        {
            if (verbose)
            {
                cat(paste("Weights cannot be used with missing data\n",
                  sep = ""))
            }
        }
        if (sparse)
        {
            if (verbose)
            {
                cat(paste("sparse option cannot be used with missing data\n",
                  sep = ""))
            }
            sparse <- FALSE
        }
    ...
    if (!NoWeights)
    {
        ...
        if (verbose)
        {
            cat(paste("Only naive DoF can be used with weighted PLS\n",
                sep = ""))
        }
    } else
    {
    ...
    if (!(modele %in% c("pls", "pls-glm-logistic", "pls-glm-family", "pls-glm-Gamma",
    ...
    if (!is.null(family))
    {
        ...
        {
            family <- get(family, mode = "function", envir = parent.frame(n = sys.nframe()))
        }
    ...
    if (scaleY)
    {
        ...
        } else
        {
            ...
            stdevY <- sqrt((length(dataY) - 1)/length(dataY) * weighted.mean((dataY -
                meanY)^2, weights))
    ...
    if (scaleX)
    {
        ...
        } else
        {
            ...
            stdevX <- sqrt((length(dataY) - 1)/length(dataY) * apply((sweep(dataX,
                2, meanX))^2, 2, weighted.mean, weights))
            ExpliX <- sweep(sweep(dataX, 2, meanX), 2, stdevX, "/")
            attr(ExpliX, "scaled:center") <- meanX
            attr(ExpliX, "scaled:scale") <- stdevX
        }

        # On centre et on réduit PredictY
        if (PredYisdataX)
        {
            PredictY <- ExpliX
        } else
        {
            PredictY <- sweep(sweep(dataPredictY, 2, attr(ExpliX, "scaled:center")),
                2, attr(ExpliX, "scaled:scale"), "/")
        }
    } else
    {
        ExpliX <- dataX
    ...
    #On copie ExpliX dans XXwotNA en tant que matrice, et on ne garde pas les na
  ...
  res <- list(nr = nrow(ExpliX), nc = ncol(ExpliX), nt = nt, ww = NULL,
        wwnorm = NULL, wwetoile = NULL, tt = NULL, pp = NULL, CoeffC = NULL,
        uscores = NULL, YChapeau = NULL, residYChapeau = NULL, RepY = RepY,
        na.miss.Y = na.miss.Y, YNA = YNA, residY = RepY, ExpliX = ExpliX,
        na.miss.X = na.miss.X, XXNA = XXNA, residXX = ExpliX, PredictY = PredictYwotNA,
        RSS = rep(NA, nt), RSSresidY = rep(NA, nt), R2 = rep(NA, nt), R2residY = rep(NA, nt),
        press.ind = NULL, press.tot = NULL, Q2cum = rep(NA, nt),
        family = family, ttPredictY = NULL, typeVC = typeVC, dataX = dataX,
        dataY = dataY)
    if (NoWeights)
    {
        res$weights <- rep(1L, res$nr)
    } else
    {
        res$weights <- weights
    }
    ...
    if (modele == "pls")
    {
        if (scaleY)
        {
            res$YChapeau <- rep(attr(RepY, "scaled:center"), nrow(ExpliX))
    ...
    for (kk in 1:nt)
    {
        ...
        if (any(temptest < tol_Xi))
        {
            ...
            if (is.null(names(which(temptest < tol_Xi))))
            {
                if (verbose)
                {
                  cat(paste("Warning : ", paste(names(which(temptest <
                    tol_Xi)), sep = "", collapse = " "), " < 10^{-12}\n",
                    sep = ""))
                }
            } else
            {
                if (verbose)
                {
                  cat(paste("Warning : ", paste((which(temptest < tol_Xi)),
                    sep = "", collapse = " "), " < 10^{-12}\n", sep = ""))
                }
            }
            if (verbose)
        ...
        if (modele == "pls")
        {
            # On normalise en fonction de si les vecteurs possèdent un poid, ou non
            if (NoWeights)
            {
            ...
            if (!NoWeights)
            {
                tempww <- t(XXwotNA * weights) %*% YwotNA/(t(XXNA * weights) %*%
                  YwotNA^2)
            }
            # Si on veut que les valeurs p doivent calibrer le modèle ?
            if (pvals.expli)
            {
                ...
                if (sparse)
                {
                  # Si c'est vrai au moins une fois, on fait passer les cases ou c'est faux à 0
        ...
        if (modele %in% c("pls-glm-family", "pls-glm-Gamma", "pls-glm-gaussian",
            ...
            if (!pvals.expli)
            {
              tempww <-  foreach(jj=1:(res$nc), .combine=c) %dopar% {
                  coef(fastglm::fastglm(y=YwotNA, x=cbind(res$tt, XXwotNA[,jj]), family = family))[kk]
                }
                XXwotNA[!XXNA] <- 0
            ...
            } else
            {
                ...
                tmpList <- foreach(jj=1:(res$nc), .combine='cbind',.multicombine=TRUE) %dopar% {
                  ...
                  tmww <- summary(fastglm::fastglm(y=YwotNA, x=cbind(res$tt, XXwotNA[,jj]), family = family))$coefficients[kk,]
        ...
        if (modele %in% c("pls-glm-polr"))
        {
            ...
            if (!pvals.expli)
            {
                ...
                tempww<-foreach(jj = 1:(res$nc), .combine=c) %dopar%
                {
                  -1 * MASS::polr(YwotNA ~ cbind(tts, XXwotNA[,
                    jj]), na.action = na.exclude, method = method)$coef[kk]
                }
                XXwotNA[!XXNA] <- 0
                rm(tts)
            } else
            {
                ...
                tmpList <- foreach(jj=1:(res$nc), .combine='cbind',.multicombine=TRUE) %dopar%
                {
                  tmww <- -1 * summary(MASS::polr(YwotNA ~ cbind(tts, XXwotNA[,
                    jj]), na.action = na.exclude, Hess = TRUE, method = method))$coefficients[kk,
                    ]
                  tmpval<-2 * pnorm(-abs(tmww[3]))
                  c(tmww[1],tmpval,(tmpval<alpha.pvals.expli))
        ...
        if ((break_nt_sparse) & (kk == 1L))
        {
            if (verbose)
            {
                cat(paste("No significant predictors (<", alpha.pvals.expli,
                  ") found\n", sep = ""))
            }
            if (verbose)
            {
        ...
        if ((break_nt_sparse) & !(kk == 1L))
        {
            ...
            if (!(break_nt_sparse1))
            {
                if (verbose)
                {
                  cat(paste("No more significant predictors (<", alpha.pvals.expli,
                    ") found\n", sep = ""))
                }
                if (verbose)
                {
        ...
        # Normalisation du vecteur a1 (en vecteur w)
        tempwwnorm <- tempww/sqrt(drop(crossprod(tempww)))

        # Détermination du composant t
        ...
        if (na.miss.X & !na.miss.Y)
        {
            if (sparse == FALSE)
            {
                ...
                for (ii in 1:res$nr)
                {
                  ...
                  # On vérifie si on est en dessous du seuil tol_Xi
                  if (rcond(t(cbind(res$pp, temppp)[XXNA[ii, ], , drop = FALSE]) %*%
                    cbind(res$pp, temppp)[XXNA[ii, ], , drop = FALSE]) <
                    tol_Xi)
                    {
                    break_nt <- TRUE

                    # Nombre de composants nt
                    res$computed_nt <- kk - 1
                    if (verbose)
                    {
                      cat(paste("Warning : reciprocal condition number of t(cbind(res$pp,temppp)[XXNA[",
                        ii, ",],,dr op=FALSE])%*%cbind(res$pp,temppp)[XXNA[",
                        ii, ",],,drop=FALSE] < 10^{-12}\n", sep = ""))
                    }
                    if (verbose)
                    {
        ...
        if (!PredYisdataX)
        {
            if (na.miss.PredictY & !na.miss.Y) # Si il manque des données à la liste des prédictions mais pas dans les données réelles
            {
                if (sparse == FALSE)
                {
                  for (ii in 1:nrow(PredictYwotNA))
                  {
                    if (rcond(t(cbind(res$pp, temppp)[PredictYNA[ii, ],
                      , drop = FALSE]) %*% cbind(res$pp, temppp)[PredictYNA[ii,
                      ], , drop = FALSE]) < tol_Xi)
                      {
                      break_nt <- TRUE
                      res$computed_nt <- kk - 1
                      if (verbose)
                      {
                        cat(paste("Warning : reciprocal condition number of t(cbind(res$pp,temppp)[PredictYNA[",
                          ii, ",,drop=FALSE],])%*%cbind(res$pp,temppp)[PredictYNA[",
                          ii, ",,drop=FALSE],] < 10^{-12}\n", sep = ""))
                      }
                      if (verbose)
                      {
        ...
        # On rattache les résultat à la variable de sortie
        ...
        ############################################## # Computation of the coefficients # of the model with kk components #
        ############################################## #
        ...
        if (modele == "pls")
        {
            ...
            } else
            {
                if (!(na.miss.X | na.miss.Y))
                {
                  tempCoeffC <- c(rep(0, kk - 1), solve(t(res$tt[YNA, kk]) %*%
                    res$tt[YNA, kk]) %*% t(res$tt[YNA, kk]) %*% YwotNA[YNA])
                  tempCoeffConstante <- 0
                  res$CoeffCFull <- cbind(res$CoeffCFull, c(tempCoeffC,
                    rep(NA, nt - kk)))
                } else
                {
                  tempCoeffC <- c(rep(0, kk - 1), solve(t(res$tt[YNA, kk]) %*%
            ...
            res$Std.Coeffs <- rbind(tempCoeffConstante, res$wwetoile %*%
                res$CoeffC)
        ...
        if (modele %in% c("pls-glm-family", "pls-glm-Gamma", "pls-glm-gaussian",
            ...
            if (kk == 1)
            {
                ...
                res$Coeffsmodel_vals <- rbind(summary(tempconstglm)$coefficients,
                  matrix(rep(NA, 4 * nt), ncol = 4))
                res$ChisqPearson <- crossprod(residuals.glm(tempconstglm,
                  type = "pearson"))
                # if ((modele %in% c('pls-glm-logistic'))|(family$family=='binomial'))
                # {
                res$MissClassed <- sum(unclass(res$RepY) != ifelse(predict(tempconstglm,
                ...
                res$Coeffsmodel_vals <- cbind(res$Coeffsmodel_vals, rbind(summary(tempregglm)$coefficients,
                  matrix(rep(NA, 4 * (nt - kk)), ncol = 4)))
                res$ChisqPearson <- c(res$ChisqPearson, crossprod(residuals.glm(tempregglm,
                  type = "pearson")))
                # if ((modele %in% c('pls-glm-logistic'))|(family$family=='binomial'))
                # {
                res$MissClassed <- cbind(res$MissClassed, sum(unclass(res$RepY) !=
                  ifelse(predict(tempregglm, type = "response") < 0.5,
                    0, 1)))
                # }
        ...
        if (modele %in% c("pls-glm-polr"))
        {
            ...
            {
                diag(piVaryy[-length(piVaryy)]) - piVaryy[-length(piVaryy)] %*%
                  t(piVaryy[-length(piVaryy)])
            }
            Chisqcomp <- function(yichisq, pichisq)
            {
                t(yichisq[-length(yichisq)] - pichisq[-length(pichisq)]) %*%
                  MASS::ginv(Varyy(pichisq)) %*% (yichisq[-length(yichisq)] -
                  pichisq[-length(pichisq)])
            }
            Chiscompmatrix <- function(rowspi, rowsyi)
            {
                sum(mcmapply(Chisqcomp, rowsyi, rowspi))
            }

            if (kk == 1)
            {
                tempconstpolr <- MASS::polr(YwotNA ~ 1, na.action = na.exclude,
                  Hess = TRUE, method = method)
                res$AIC <- AIC(tempconstpolr)
                res$BIC <- AIC(tempconstpolr, k = log(res$nr))
                res$MissClassed <- sum(!(unclass(predict(tempconstpolr,
                  type = "class")) == unclass(res$RepY)))
                res$Coeffsmodel_vals <- rbind(summary(tempconstpolr)$coefficients,
                  matrix(rep(NA, 3 * nt), ncol = 3))
                ...
                res$ChisqPearson <- sum(Chiscompmatrix(as.list(as.data.frame(t(predict(tempconstpolr,
                  type = "probs")))), as.list(as.data.frame(t(tempmat)))))
                suppressWarnings(rm(tempconstpolr))
                tttrain <- data.frame(YwotNA = YwotNA, tt = res$tt)
                tempregpolr <- MASS::polr(YwotNA ~ ., data = tttrain, na.action = na.exclude,
                  Hess = TRUE, method = method)
                rm(tttrain)
                res$AIC <- cbind(res$AIC, AIC(tempregpolr))
                res$BIC <- cbind(res$BIC, AIC(tempregpolr, k = log(res$nr)))
                res$MissClassed <- cbind(res$MissClassed, sum(!(unclass(predict(tempregpolr,
                  type = "class")) == unclass(res$RepY))))
                res$Coeffsmodel_vals <- cbind(res$Coeffsmodel_vals, rbind(summary(tempregpolr)$coefficients,
                  matrix(rep(NA, 3 * (nt - kk)), ncol = 3)))
                tempmodord <- predict(tempregpolr, type = "class")
                tempfff <- ~tempmodord - 1
                tempm <- model.frame(tempfff, tempmodord)
                tempmat <- model.matrix(tempfff, model.frame(tempfff, tempmodord))
                res$ChisqPearson <- c(res$ChisqPearson, sum(Chiscompmatrix(as.list(as.data.frame(t(predict(tempregpolr,
                  type = "probs")))), as.list(as.data.frame(t(tempmat))))))
                tempCoeffC <- -1 * as.vector(tempregpolr$coef)
                tempCoeffConstante <- as.vector(tempregpolr$zeta)
                res$CoeffCFull <- matrix(c(tempCoeffConstante, tempCoeffC,
                  rep(NA, nt - kk)), ncol = 1)
                res$CoeffConstante <- tempCoeffConstante
            } else
            {
                ...
                {
                  tttrain <- data.frame(YwotNA = YwotNA, tt = res$tt)
                  tempregpolr <- MASS::polr(YwotNA ~ ., data = tttrain,
                    na.action = na.exclude, Hess = TRUE, method = method)
                  rm(tttrain)
                  res$AIC <- cbind(res$AIC, AIC(tempregpolr))
                  res$BIC <- cbind(res$BIC, AIC(tempregpolr, k = log(res$nr)))
                  res$MissClassed <- cbind(res$MissClassed, sum(!(unclass(predict(tempregpolr,
                  ...
                  tempfff <- ~tempmodord - 1
                  tempm <- model.frame(tempfff, tempmodord)
                  tempmat <- model.matrix(tempfff, model.frame(tempfff,
                    tempmodord))
                  res$ChisqPearson <- c(res$ChisqPearson, sum(Chiscompmatrix(as.list(as.data.frame(t(predict(tempregpolr,
                    type = "probs")))), as.list(as.data.frame(t(tempmat))))))
                  tempCoeffC <- -1 * as.vector(tempregpolr$coef)
                  tempCoeffConstante <- as.vector(tempregpolr$zeta)
                  res$CoeffCFull <- cbind(res$CoeffCFull, c(tempCoeffConstante,
                    tempCoeffC, rep(NA, nt - kk)))
                  res$CoeffConstante <- cbind(res$CoeffConstante, tempCoeffConstante)
                } else
                {
                  tttrain <- data.frame(YwotNA = YwotNA, tt = res$tt)
            ...
            res$wwetoile <- (res$wwnorm) %*% solve(t(res$pp) %*% res$wwnorm)
            res$CoeffC <- tempCoeffC
            res$Std.Coeffs <- as.matrix(rbind(as.matrix(tempCoeffConstante),
                res$wwetoile %*% res$CoeffC))
            rownames(res$Std.Coeffs) <- c(names(tempregpolr$zeta), colnames(ExpliX))
        }



        ############################################## # Prediction of the components # as if missing values (model free) #
        ...
        if (!(na.miss.X | na.miss.Y))
        {
            ...
            ############################################## # Cross validation # without missing value # #
            ...
            if (modele == "pls")
            {
                ...
                if (kk == 1)
                {
                  ...
                  if (!NoWeights)
                  {
                    res$RSSresidY <- crossprod(RepY - mean(RepY), weights *
                      (RepY - mean(RepY)))
                  }
                }
                if (NoWeights)
                {
                  res$RSSresidY <- cbind(res$RSSresidY, crossprod(res$residY -
                    res$residYChapeau))
                }
                if (!NoWeights)
                {
                  res$RSSresidY <- cbind(res$RSSresidY, crossprod(res$residY -
                    res$residYChapeau, weights * (res$residY - res$residYChapeau)))
                }


                tempCoeffs <- res$wwetoile %*% res$CoeffC * attr(res$RepY,
                  "scaled:scale")/attr(res$ExpliX, "scaled:scale")
                tempConstante <- attr(res$RepY, "scaled:center") - sum(tempCoeffs *
                  attr(res$ExpliX, "scaled:center"))
                res$Coeffs <- rbind(tempConstante, tempCoeffs)

                res$YChapeau <- attr(res$RepY, "scaled:center") + attr(res$RepY,
                  "scaled:scale") * res$tt %*% res$CoeffC
                ...
                if (kk == 1)
                {
                  ...
                  if (!NoWeights)
                  {
                    res$RSS <- crossprod(dataY - mean(dataY), weights *
                      (dataY - mean(dataY)))
                  }
                }
                if (NoWeights)
                ...
                if (!NoWeights)
                {
                  res$RSS <- cbind(res$RSS, crossprod(res$Yresidus, weights *
                    res$Yresidus))
                }
            }
            ##############################################
            ...
            if (modele %in% c("pls-glm-family", "pls-glm-Gamma", "pls-glm-gaussian",
                ...
                tempCoeffs <- res$wwetoile %*% res$CoeffC * attr(res$RepY,
                  "scaled:scale")/attr(res$ExpliX, "scaled:scale")
                tempConstante <- attr(res$RepY, "scaled:center") - sum(tempCoeffs *
                  attr(res$ExpliX, "scaled:center")) + attr(res$RepY, "scaled:scale") *
                  res$Std.Coeffs[1]
                res$Coeffs <- rbind(tempConstante, tempCoeffs)
        ...
        } else
        {
            if (na.miss.X & !na.miss.Y)
            {
                ...
                ############################################## # Cross validation # with missing value(s) # #
                ...
                if (modele %in% c("pls-glm-family", "pls-glm-Gamma", "pls-glm-gaussian",
                  ...
                  tempCoeffs <- res$wwetoile %*% res$CoeffC * attr(res$RepY,
                    "scaled:scale")/attr(res$ExpliX, "scaled:scale")
                  tempConstante <- attr(res$RepY, "scaled:center") - sum(tempCoeffs *
                    attr(res$ExpliX, "scaled:center")) + attr(res$RepY,
                    "scaled:scale") * res$Std.Coeffs[1]
                  res$Coeffs <- rbind(tempConstante, tempCoeffs)
        ...
        if (modele == "pls")
        {
            ...
            if (kk == 1)
            {
                ...
                if (MClassed)
                {
                  res$MissClassed <- sum(unclass(dataY) != ifelse(predict(lm(dataY ~
                    1, weights = res$weights)) < 0.5, 0, 1))
                  res$MissClassed <- cbind(res$MissClassed, sum(unclass(dataY) !=
                    ifelse(res$YChapeau < 0.5, 0, 1)))
    ...
    {
        res$Coeffsmodel_vals <- res$Coeffsmodel_vals[1:(dim(res$Coeffsmodel_vals)[1] -
            (nt - res$computed_nt)), ]
    }
    ...
    ############################################## # Computing RSS, PRESS, # Chi2, Q2 and Q2cum # #
    ...
    if (modele == "pls")
    {
        ...
        if (MClassed == FALSE)
        {
            res$InfCrit <- t(rbind(res$AIC, res$RSS, c(NA, res$R2), c(NA,
                res$R2residY), res$RSSresidY, res$AIC.std))
            dimnames(res$InfCrit) <- list(paste("Nb_Comp_", 0:res$computed_nt,
                sep = ""), c("AIC", "RSS_Y", "R2_Y", "R2_residY", "RSS_residY",
                "AIC.std"))
            res$ic.dof <- infcrit.dof(res, naive = naive)
            res$InfCrit <- cbind(res$InfCrit, res$ic.dof)
        } else
        {
            res$InfCrit <- t(rbind(res$AIC, res$RSS, c(NA, res$R2), res$MissClassed,
                c(NA, res$R2residY), res$RSSresidY, res$AIC.std))
            dimnames(res$InfCrit) <- list(paste("Nb_Comp_", 0:res$computed_nt,
                sep = ""), c("AIC", "RSS_Y", "R2_Y", "MissClassed", "R2_residY",
                "RSS_residY", "AIC.std"))
            res$ic.dof <- infcrit.dof(res, naive = naive)
            res$InfCrit <- cbind(res$InfCrit, res$ic.dof)
        }
    ...
    if (modele %in% c("pls-glm-family", "pls-glm-Gamma", "pls-glm-gaussian",
        ...
        res$R2 <- 1 - res$RSS[2:(res$computed_nt + 1)]/res$RSS[1]
        if (modele %in% c("pls-glm-family", "pls-glm-Gamma", "pls-glm-gaussian",
            "pls-glm-inverse.gaussian", "pls-glm-poisson"))
            {
            res$InfCrit <- t(rbind(res$AIC, res$BIC, res$ChisqPearson,
                res$RSS, c(NA, res$R2), c(NA, res$R2residY), res$RSSresidY))
            dimnames(res$InfCrit) <- list(paste("Nb_Comp_", 0:res$computed_nt,
                sep = ""), c("AIC", "BIC", "Chi2_Pearson_Y", "RSS_Y", "R2_Y",
        ...
        if ((modele %in% c("pls-glm-logistic")) | (family$family == "binomial"))
        {
            res$InfCrit <- t(rbind(res$AIC, res$BIC, res$MissClassed, res$ChisqPearson,
                res$RSS, c(NA, res$R2), c(NA, res$R2residY), res$RSSresidY))
            dimnames(res$InfCrit) <- list(paste("Nb_Comp_", 0:res$computed_nt,
                sep = ""), c("AIC", "BIC", "Missclassed", "Chi2_Pearson_Y",
    ...
    if (modele == "pls")
    {
        ...
        res$ValsPredictY <- attr(res$RepY, "scaled:center") + attr(res$RepY,
            "scaled:scale") * res$ttPredictY %*% res$CoeffC

        res$Std.XChapeau <- res$tt %*% t(res$pp)
        rownames(res$Std.XChapeau) <- rownames(ExpliX)
        if (EstimXNA)
        {
            res$XChapeau <- sweep(sweep(res$Std.XChapeau, 2, attr(res$ExpliX,
                "scaled:scale"), FUN = "*"), 2, attr(res$ExpliX, "scaled:center"),
                FUN = "+")
            rownames(res$XChapeau) <- rownames(ExpliX)
            colnames(res$XChapeau) <- colnames(ExpliX)

            res$XChapeauNA <- sweep(sweep(res$Std.XChapeau, 2, attr(res$ExpliX,
                "scaled:scale"), FUN = "*"), 2, attr(res$ExpliX, "scaled:center"),
                FUN = "+") * (!XXNA)
            rownames(res$XChapeau) <- rownames(ExpliX)
            colnames(res$XChapeau) <- colnames(ExpliX)
