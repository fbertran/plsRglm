...
#' 
#' @export permcoefs.plsRglm.raw
permcoefs.plsRglm.raw <- function(dataset, ind, nt, modele, family = NULL, fit_backend = "stats", maxcoefvalues,ifbootfail,verbose){
    tempcoefs <- try(PLS_glm_wvc(dataY = dataset[ind, 1], dataX = dataset[, 
        -1], nt = nt, modele = modele, family=family, fit_backend = fit_backend, keepcoeffs = TRUE, verbose=verbose)$coeffs, silent=TRUE)
    Cond <- FALSE
